...
async def transcribe_uploaded_media(
    *,
    ...
    try:
        return await _transcribe_media_path(
            source_path=temp_path,
            source_name=request.source_name,
            silence_detect=request.silence_detect,
            ...
            verbose=verbose,
            likely_reason_details_fn=likely_reason_details_fn,
...
async def transcribe_local_media_path(
    *,
    ...
    if not source_path.is_absolute():
        raise UploadedMediaError(422, "Invalid source path.", ["Path must be absolute."])
    if not source_path.exists():
        raise UploadedMediaError(404, "Local file not found.", [f"Path not found: {source_path}"])
    if source_path.is_dir():
        raise UploadedMediaError(422, "Invalid source path.", ["Path must point to a file."])
    return await _transcribe_media_path(
...
async def _transcribe_media_path(
    *,
    source_path: Path,
    source_name: str,
    silence_detect: bool,
    ...
    try:
        audio, sample_rate = decode_media_file(source_path)
    except ValueError as exc:
        detail = f"No decodable audio stream found in '{source_name}'."
